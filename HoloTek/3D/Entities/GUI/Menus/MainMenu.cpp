...
std::future<void> HoloTek::MainMenu::InitializeMenuAsync()
{
	...
	auto background = std::make_unique<Panel>(m_devicesResources, m_scene, float2(0.35f, 0.17f), float4(0.85f, 0.42f, 0.27f, 0.6f));
	...
	background->AddGUIEntity(std::move(buttonRefreshActivities), { -0.1f, 0.0f });
	background->AddGUIEntity(std::move(buttonStopDetection), { 0.1f, 0.0f });
