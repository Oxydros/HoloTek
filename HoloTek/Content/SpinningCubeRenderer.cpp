...
#pragma optimize( "", off )
std::future<void> SpinningCubeRenderer::CreateDeviceDependentResourcesAsync()
{
	...
	std::vector<byte> pixelShaderFileData = co_await DX::ReadDataAsync(L"ms-appx:///CubePixelShader.cso");
	winrt::check_hresult(
	...
	if (!m_usingVprtShaders)
	{
		...
		std::vector<byte> geometryShaderFileData = co_await DX::ReadDataAsync(L"ms-appx:///CubeGeometryShader.cso");
