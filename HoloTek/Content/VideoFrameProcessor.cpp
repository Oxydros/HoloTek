...
std::future<std::shared_ptr<VideoFrameProcessor>> VideoFrameProcessor::CreateAsync(void)
{
	...
	winrt::agile_ref<MediaCapture> mediaCapture;

	co_await mediaCapture.get().InitializeAsync(settings);

	MediaFrameSource selectedSource = mediaCapture.get().FrameSources().Lookup(selectedSourceInfo.Id());
	MediaFrameReader reader = co_await mediaCapture.get().CreateFrameReaderAsync(selectedSource);
	...
	{
		co_return std::make_shared<VideoFrameProcessor>(mediaCapture, reader, selectedSource);
	}
	else
	{
		co_return std::shared_ptr<VideoFrameProcessor>(nullptr);
